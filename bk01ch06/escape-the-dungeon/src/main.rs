    ...
    while health > 0 && !escaped {
        ...
        match choice.as_str() {
            "left" | "l" => {
                ...
                loop {
                    println!(
                        "A riddle is carved on the wall: \"Two doors: one lies, one tells truth.\""
                    );
                    ...
                    if door == "1" {
                        ...
                        println!(
                            "The door opens smoothly. A friendly rat hands you a coin. (+1 treasure)"
                        );
                        treasure += 1;
                        break;
                    } else {
            ...
            "right" | "r" => {
                println!(
                    "You stride right into a damp chamber. Something glitters under murky water."
                );
                ...
                if action == "g" || action == "glitter" {
                    println!(
                        "You reach in... Something bites! (-1 health) But you grab a gem! (+2 treasure)"
                    );
                    health -= 1;
                    treasure += 2;
                } else if action == "s" || action == "safe" {
                    println!(
                        "You keep your hands clean. Wise... but you find a single coin near the edge. (+1 treasure)"
                    );
                    treasure += 1;
                } else {
            ...
            "wait" | "w" => {
                ...
                println!(
                    "The inscription reads: \"The code is the first odd number greater than 6.\""
                );
